...
const REPEATPLAYLIST = true;

Utils.log('playlist.js:getSettingsFilePath', PreferencesModel.getLocation());

var Playlist = {
  get: function get () {
    Utils.log('playlist.js:getSettingsFilePath', PreferencesModel.getLocation());
    var pls = PreferencesModel.get('streams.' + STREAMNAME + '.playlist');
    // stream = stream || '0';
    if (!pls) {
  ...
  save: function save (fullPlaylist) {
    ...
    return PreferencesModel.set('streams.' + STREAMNAME + '.playlist', fullPlaylist);
  },
  ...
  getDisplayTitle: function getDisplayTitle (entry) {
    ...
    // console.log('getDisplayTitle', entry, displayTitle)
    return displayTitle;
  },
  getActive: function setActive(index) {
    ...
    console.log('found active at ', index + 1);
  ...
  setActive: function setActive(index) {
    ...
    if (index === 'next' || index === 'prev' || index === 'active') {
      ...
      if (index === 'next') {
        ...
        if (REPEATPLAYLIST && (index > playlist.length -1)) {
          console.log('Repeat Active - looping playlist');
          index = 0;
        }
      }
      if (index === 'prev') {
