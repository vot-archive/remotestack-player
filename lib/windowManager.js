...
var presets = {
  main: {width: 300, height: 110, minWidth: 250, minHeight: /*110*/35, frame: false, custom: {showOnReady: true}},
...
function create (name, opts) {
  ...
  if (showOnReady) {
    instances[name].once('ready-to-show', function () {
      ...
      var oldPosition = settings.getSync('settings.ui.windows.' + name + '.position')
      var oldSize = settings.getSync('settings.ui.windows.' + name + '.size')
  ...
  */
  ...
  instances[name].on('resize',  function () {
    ...
    if (oldSize) settings.setSync('settings.ui.windows.' + name + '.size', oldSize);
  });

  instances[name].on('move',  function () {
    var oldPosition = instances[name].getPosition();
    if (oldPosition) settings.setSync('settings.ui.windows.' + name + '.position', oldPosition);
  });
