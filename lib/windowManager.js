...
var presets = {
  main: {width: 320, height: 420, minWidth: 260, minHeight: 35, frame: false, custom: {showOnReady: true}},
...
function create (name, opts) {
  ...
  if (process.platform === 'windows') {
    windowOpts.icon = path.join(__dirname, '../views/assets/icons/player.ico');
  }
  if (process.platform === 'linux') {
    windowOpts.icon = path.join(__dirname, '../views/assets/icons/player-256.png');
  }
  if (process.platform === 'darwin') {
    windowOpts.icon = path.join(__dirname, '../views/assets/icons/player.icns');
  }
  ...
  */
  ...
  instances[name].uuid = Utils.uuid();
