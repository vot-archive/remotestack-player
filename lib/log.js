...
// const logpath = path.join(process.appdata || approot, filename);
const logpath = path.join(approot, filename);

const runningFromCLI = process.env.PWD && process.env._ && process.env._.endsWith('/electron');

const ARCHIVE_LOGS = 3;
const WRITE_TO_STDOUT = runningFromCLI ? true : false;
const WRITE_TO_FILE = runningFromCLI ? false : true;
